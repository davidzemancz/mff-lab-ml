...
We have seen gradual development of machine learning systems to neural networks.
...
~~~
- In the seeked minimum, $∇_{→x} f(→x)$ must also be orthogonal to the constraint
...
~~~
Consequently, the seeked minimum either fulfils $∇_{→x} f - λ∇_{→x} g = 0$ for some $λ$,
...
Assume we want to find a categorical distribution $p_1, …, p_n$ with maximum
entropy.
...
the entropy $H[⋅]$. To minimize a functional with respect fo a function,
...
We can verify that setting $λ_1 = 1 - \log \sqrt{2πσ^2}$, $λ_2=0$ and $λ_3=-1/(2σ^2)$
fulfils all the constraints, arriving at
$$\begin{aligned}
  p(x) &= \exp\Big(λ_1 + λ_2 x + λ_3 (x-μ)^2 - 1\Big) \\
       &= \exp\left(1 - \log \sqrt{2πσ^2} + -1/(2σ^2) (x-μ)^2 - 1\right) \\
...
$$\begin{gathered} \min(p, r) ≤ AM(p, r), \\ AM(p, r) ≤ \max(p, r).\end{gathered}$$
...
Geometric mean of precision recall is
$$HM(p, r) ≝ \frac{2}{\frac{1}{p} + \frac{1}{r}}.$$

In addition to being bounded by the input values, it is also
dominated by the minimum of its input values:
$$\begin{gathered} \min(p, r) ≤ HM(p, r), \\ HM(p, r) ≤ \max(p, r), \\ HM(p, r) ≤ 2\min(p, r).\end{gathered}$$
