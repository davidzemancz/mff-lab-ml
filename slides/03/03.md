...
predictions), because multiplying $→w$ by a constant does not change prediction
and the loss derivative does not depend on $→w$. Note that the _second_
condition is crucial; the first holds also for logistic regression, but the
learning rate matters there.
